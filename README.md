...
> 📌 **Data Citation:** This project uses a **fully synthetic dataset** generated by the team using [`data/synthetic_generator.py`](data/synthetic_generator.py). No proprietary, confidential, or copyrighted data was used. The dataset schema, field definitions, assumptions, and data generation logic are fully documented here as required by the submission guidelines.

### Why Synthetic Data?

Real AML transaction datasets with labeled fraud cases are not publicly available due to banking privacy and regulatory restrictions. We generated a synthetic dataset that:
- Mirrors real banking transaction distributions across 7 payment categories
- Plants labeled AML fraud patterns using known real-world typologies (structuring, smurfing, layering, rapid cashout, round-tripping)
- Includes **"hard negative" edge cases** (e.g., legitimate cash flows that look like structuring) to prevent the ML model from learning trivially separable patterns
- Intentionally bridges fraud accounts into the normal transaction network to simulate realistic money mule behavior

### Data Generation Logic (from `data/synthetic_generator.py`)

**Phase 1 — Fraud Pattern Generation** (run first to seed fraud account pool):

| AML Pattern | How It Was Generated |
| :--- | :--- |
| **Structuring** | 8 cases × 8–18 transactions per case. Each amount randomly drawn from $8,200–$9,800 (just below $10,000 CTR threshold). Timestamps spaced 0.5–6 hours apart. |
| **Smurfing (Fan-Out)** | 6 cases. One source account fans out to 5–12 mule accounts using Dirichlet-distributed splits of $50k–$200k. Mules then re-aggregate to a collector. |
| **Layering (Chains)** | 6 cases. Money moves A→B→C→D→E through chains of 4–7 accounts. Each hop reduces amount by 1–8% to simulate fee skimming. |
| **Rapid Cash-Out** | 10 cases. Large deposit ($10k–$80k) followed by withdrawal within 0.25–1.5 hours to a different account. |
| **Round-Tripping** | 5 cases. Money flows in a closed loop of 3–5 accounts (A→B→C→A), eroding 1–5% per hop. |

**Phase 2 — Normal Transaction Generation** (links to fraud accounts for realism):
- 10,000 base transactions across 7 categories (payroll, retail, bills, p2p, mortgage, tuition, business)
- 15% of senders/receivers randomly picked from the fraud account pool to create realistic network overlap
- Hard negatives included: legitimate cash structuring look-alikes and rapid treasury sweeps
...
| **AML Typologies** | structuring, smurfing, layering, rapid\_cashout, round\_tripping |
...
| :--- | :--- | :--- |
| `transaction_id` | string | Unique 8-character UUID per transaction |
| `timestamp` | datetime | Exact date and time of transaction |
| `sender_id` | string | Sending account ID (format: `ACC#####`) |
| `receiver_id` | string | Receiving account ID (format: `ACC#####`) |
| `amount` | float | Transaction amount in USD |
| `category` | string | Payment type (retail, payroll, p2p, bills, business, tuition, mortgage, transfer, withdrawal) |
| `is_fraud` | int | Ground truth: 1 = fraudulent, 0 = normal |
| `typology` | string | AML fraud pattern name or "normal" |
| `case_id` | string | Groups related transactions in same fraud case |
